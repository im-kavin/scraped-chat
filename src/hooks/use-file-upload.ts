...
export const useFileUpload = (
	...
	const validateFile = useCallback(
		// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: demo not important
	...
	const addFiles = useCallback(
		...
		(newFiles: FileList | File[]) => {
			...
			if (validFiles.length > 0) {
				...
				setState((prev) => {
					const newFilesResult = multiple ? [...prev.files, ...validFiles] : validFiles;
					onFilesChange?.(newFilesResult);
					return {
						...prev,
						files: newFilesResult,
						errors,
					};
				});
